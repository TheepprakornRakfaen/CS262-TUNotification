...
# Solution
ระบบ TUNotification ถูกพัฒนาขึ้นเพื่อแก้ไขปัญหามลภาวะทางเสียงภายในมหาวิทยาลัย โดยใช้เทคโนโลยี IoT ร่วมกับ Cloud Services บน AWS เพื่อให้สามารถตรวจจับ วิเคราะห์ และแจ้งเตือนปัญหาเสียงรบกวนได้แบบ Real-time อุปกรณ์ IoT ที่ติดตั้งภายในพื้นที่ จะทำหน้าที่ตรวจวัดระดับเสียงผ่านเซนเซอร์ INMP441 และส่งข้อมูลเข้าสู่ระบบ Cloud ผ่าน ESP32 จากนั้นข้อมูลจะถูกประมวลผลด้วยบริการต่างๆบน AWS เพื่อจัดเก็บ วิเคราะห์ และส่งต่อไปยัง Web Application ผู้ใช้งานสามารถตรวจสอบตำแหน่งและระดับความรุนแรงของเสียงรบกวนผ่านหน้าเว็บไซต์ที่เชื่อมต่อกับ Google Maps API ได้แบบ Real-time ทำให้สามารถระบุจุดเกิดปัญหาได้อย่างแม่นยำ นอกจากนี้ ระบบยังสามารถต่อยอดไปสู่การแจ้งเตือนผ่าน LINE Notification หรือระบบแจ้งเตือนอื่นๆ เพื่อช่วยให้เจ้าหน้าที่สามารถเข้าจัดการปัญหาได้รวดเร็วขึ้น

# AWS Architecture

<img width="1060" height="953" alt="IMG_2597" src="https://github.com/user-attachments/assets/ccb9b0ee-4c05-4e5f-b839-2c589d823e69" /> <br>
ระบบถูกออกแบบในรูปแบบ Serverless Architecture บน AWS เพื่อรองรับการทำงานแบบ Real-time และลดภาระในการดูแล Server โดยมีองค์ประกอบหลักดังนี้

### 1. IoT Device Layer
อุปกรณ์ IoT เช่น ESP32 และเซนเซอร์ INMP441 ทำหน้าที่ตรวจวัดระดับเสียงภายในพื้นที่ และส่งข้อมูลเข้าสู่ AWS Cloud อย่างต่อเนื่อง
### 2. AWS IoT Core
AWS IoT Core ทำหน้าที่เป็นตัวกลางในการรับข้อมูลจากอุปกรณ์ IoT ผ่านโปรโตคอล MQTT ช่วยให้สามารถเชื่อมต่ออุปกรณ์จำนวนมากได้อย่างมีประสิทธิภาพ
### 3. IoT Rule
เมื่อ AWS IoT Core ได้รับข้อมูลเสียง ระบบจะใช้ IoT Rule เพื่อกำหนดเส้นทางของข้อมูล และส่งต่อไปยังบริการอื่นภายใน AWS
### 4. Amazon Kinesis Data Stream
ข้อมูลเสียงที่ได้รับจะถูกส่งเข้าสู่ Amazon Kinesis Data Stream เพื่อรองรับการประมวลผลข้อมูลแบบ Streaming และ Real-time ช่วยให้ระบบสามารถจัดการข้อมูลจำนวนมากจากหลายอุปกรณ์พร้อมกันได้
### 5. AWS Lambda
AWS Lambda ทำหน้าที่ประมวลผลข้อมูลที่ได้รับจาก Kinesis เช่น วิเคราะห์ระดับเสียง คำนวณความรุนแรง หรือเตรียมข้อมูลก่อนจัดเก็บลงฐานข้อมูล โดยไม่จำเป็นต้องดูแล Server เอง
### 6. DynamoDB
ข้อมูลเสียงและตำแหน่งต่าง ๆ จะถูกจัดเก็บใน Amazon DynamoDB ซึ่งเป็นฐานข้อมูลแบบ NoSQL ที่สามารถรองรับข้อมูลจำนวนมากและเข้าถึงข้อมูลได้รวดเร็ว
### 7. API Gateway
Amazon API Gateway ทำหน้าที่เป็นตัวกลางระหว่าง Frontend และ Backend เพื่อให้ Web Application สามารถเรียกใช้งานข้อมูลจากระบบได้อย่างปลอดภัย
### 8. AWS Amplify
AWS Amplify ใช้สำหรับ Hosting และ Deploy Web Application ของระบบ ช่วยให้ผู้ใช้งานสามารถเข้าถึงหน้าเว็บไซต์ได้ง่ายและรองรับการพัฒนาแบบ Full Stack
### 9. Google Maps API
ระบบเชื่อมต่อกับ Google Maps API เพื่อแสดงตำแหน่งของแหล่งกำเนิดเสียงบนแผนที่ ช่วยให้ผู้ใช้งานและเจ้าหน้าที่สามารถตรวจสอบพื้นที่เกิดปัญหาได้อย่างชัดเจน

### Workflow ของระบบ
1. อุปกรณ์ IoT ตรวจวัดระดับเสียง
2. ส่งข้อมูลเข้าสู่ AWS IoT Core
3. IoT Rule ส่งข้อมูลไปยัง Kinesis Data Stream
4. AWS Lambda ประมวลผลข้อมูล
5. จัดเก็บข้อมูลลง DynamoDB
6. Frontend เรียกข้อมูลผ่าน API Gateway
7. แสดงผลผ่าน Web Application และ Google Maps API
